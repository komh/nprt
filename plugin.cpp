...
static NPIdentifier sfuncAdd_id;
static char* sBar_Value = "This is Bar !";
...
ScriptablePluginObjectBase::SetProperty(NPIdentifier name,
                                        const NPVariant *value)
{
  if (name == sBar_id) {    //strcpy(sBar_Value, NPVARIANT_TO_STRING(*value).utf8characters);    sBar_Value = static_cast<char*>(malloc(value->value.stringValue.utf8length + 1));    memcpy(sBar_Value, value->value.stringValue.utf8characters, value->value.stringValue.utf8length);    sBar_Value[value->value.stringValue.utf8length] = '\0';
    return true;
  }
...
{
  return (name == sFoo_id || name == sfuncAdd_id);
}
...
ScriptablePluginObject::GetProperty(NPIdentifier name, NPVariant *result)
{
  ...
  if (name == sBar_id) {
    //static int a = 17;
    char* temp = (char*)NPN_MemAlloc(32);    strcpy(temp, sBar_Value);    STRINGZ_TO_NPVARIANT(temp, *result);
                               ...
                               uint32_t argCount, NPVariant *result)
{
  if (name == sFoo_id) {
    ...
    char* temp = (char*)NPN_MemAlloc(32);
    strcpy(temp, "FOO RETURN VALUE !");
    STRINGZ_TO_NPVARIANT(temp, *result);

    return PR_TRUE;
  }
  else if (name == sfuncAdd_id) {    int a=0, b=0,c=0;
    a = NPVARIANT_TO_CINT( args[0] );    b = NPVARIANT_TO_CINT( args[1] );    c = a + b;
    INT32_TO_NPVARIANT( c, *result );
    return PR_TRUE;  }
                                      ...
                                      NPVariant *result)
{
  ...
  char* temp = (char*)NPN_MemAlloc(32);
  strcpy(temp, "DEFAULT RETURN VALUE !");
  STRINGZ_TO_NPVARIANT(temp, *result);
  ...
  m_pScriptableObject(NULL)
{
  ...
  sfuncAdd_id = NPN_GetStringIdentifier("funcAdd");
...
{  ExecJavaScript( "funcAlert('Plugin Init')" );
  if(pNPWindow == NULL)
    return FALSE;
...
#endif
int NPVARIANT_TO_CINT(NPVariant npvar){  if( NPVARIANT_IS_DOUBLE(npvar) )  {    // in Safari,Opera all numbers seem to be doubles...    return ( (int)NPVARIANT_TO_DOUBLE(npvar));  }  else if( NPVARIANT_IS_INT32(npvar) )  {    // this happens in Firefox,Netscape,Chrome    return (NPVARIANT_TO_INT32(npvar));  }  else    return -1;}
void CPlugin::ExecJavaScript(char* func){  NPIdentifier sRunJavaScript_id;  sRunJavaScript_id = NPN_GetStringIdentifier(func);
  NPString str;
  char buff[256];  sprintf(buff, "try{%s}catch(e){} void(0);", func);  str.utf8characters = buff;  str.utf8length = strlen(str.utf8characters);
  NPVariant res;  NPN_Evaluate(m_pNPInstance, sWindowObj, &str, &res);  NPN_ReleaseVariantValue(&res);}
